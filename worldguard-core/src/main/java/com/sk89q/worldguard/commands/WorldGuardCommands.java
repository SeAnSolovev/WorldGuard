...
public class WorldGuardCommands {
    ...
    public void reload(CommandContext args, Actor sender) throws CommandException {
        ...
        if (!tasks.isEmpty()) {
            throw new CommandException("В данный момент есть незавершённые задачи. Сначала используйте /wg running для их мониторинга.");
        }
    ...
    @Command(aliases = {"report"}, desc = "Создаёт отчёт по WorldGuard", flags = "p", max = 0)
    ...
    public void report(CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        if (args.hasFlag('p')) {
            ...
            ActorCallbackPaste.pastebin(worldGuard.getSupervisor(), sender, result, "Отчёт WorldGuard: %s.report");
        }
    }

    @Command(aliases = {"profile"}, usage = "[-p] [-i <интервал>] [-t <фильтр потоков>] [<минуты>]",
            desc = "Профилировать использование CPU сервера", min = 0, max = 1,
            flags = "t:i:p")
    ...
    public void profile(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        if (args.argsLength() == 0) {
            ...
            if (minutes < 1) {
                throw new CommandException("Профилирование должно длиться не менее 1 минуты.");
            } else if (minutes > 10) {
                throw new CommandException("Максимальное время профилирования — 10 минут.");
            }
        }
        ...
        if (args.hasFlag('i')) {
            ...
            if (interval < 1 || interval > 100) {
                throw new CommandException("Интервал должен быть в диапазоне от 1 до 100 миллисекунд.");
            }
            if (interval < 10) {
                sender.printDebug("Внимание: Малый интервал может вызвать дополнительное замедление во время профилирования.");
            }
        }
        Sampler sampler;
        ...
        synchronized (this) {
            if (activeSampler != null) {
                throw new CommandException("Профилирование уже запущено! Используйте /wg stopprofile для отмены.");
            }
        ...
        Futures.addCallback(sampler.getFuture(), new FutureCallback<>() {
            ...
            public void onSuccess(Sampler result) {
                ...
                try {
                    ...
                    sender.print("Данные профилирования CPU сохранены в " + dest.getAbsolutePath());
                } catch (IOException e) {
                    sender.printError("Не удалось сохранить данные профилирования CPU: " + e.getMessage());
                }
    ...
    @Command(aliases = {"stopprofile"}, usage = "",desc = "Остановить текущее профилирование", min = 0, max = 0)
    @CommandPermissions("worldguard.profile")
    public void stopProfile(CommandContext args, final Actor sender) throws CommandException {
        synchronized (this) {
            if (activeSampler == null) {
                throw new CommandException("В данный момент профилирование CPU не запущено.");
            }
    ...
    public void flushStates(CommandContext args, Actor sender) throws CommandException {
        if (args.argsLength() == 0) {
            ...
            sender.print("Все состояния сброшены.");
        } else {
            ...
            if (player != null) {
                ...
                sender.print("Состояния игрока \"" + player.getName() + "\" сброшены.");
            }
        }
    }
    ...
    public void listRunningTasks(CommandContext args, Actor sender) throws CommandException {
        ...
        if (tasks.isEmpty()) {
            sender.print("В данный момент нет запущенных задач.");
        } else {
            tasks.sort(new TaskStateComparator());
            MessageBox builder = new MessageBox("Запущенные задачи", new TextComponentProducer());
            builder.append(TextComponent.of("Примечание: Некоторые «запущенные» задачи могут ещё ждать начала.", TextColor.GRAY));
            for (Task<?> task : tasks) {
    ...
    @Command(aliases = {"debug"}, desc = "Команды для отладки")
