...
class RegionCommandsBase {
    ...
    protected static World checkWorld(CommandContext args, Actor sender, char flag, boolean allowWorldEditOverride) throws CommandException {
        if (args.hasFlag(flag)) {
                ...
                try {
                    ...
                    if (override != null) {
                        if (sender instanceof LocalPlayer && !override.equals(((LocalPlayer) sender).getWorld())) {
                            sender.printDebug(TextComponent.of("Используется переопределение //world для команды региона: " + override.getName()));
                        }
                        return override;
                    }
            ...
            if (sender instanceof LocalPlayer) {
                ...
                throw new CommandException("Пожалуйста, укажите мир с помощью -" + flag + " имя_мира.");
            }
        }
    }
    ...
    protected static String checkRegionId(String id, boolean allowGlobal) throws CommandException {
        ...
        if (!allowGlobal && id.equalsIgnoreCase("__global__")) { // Sorry, no global
            throw new CommandException(
                    "Извините, здесь нельзя использовать __global__.");
        }
    ...
    protected static ProtectedRegion checkRegionStandingIn(RegionManager regionManager, LocalPlayer player, boolean allowGlobal, String rgCmd) throws CommandException {
        ...
        if (set.size() == 0) {
            if (allowGlobal) {
                ...
                player.printDebug("Вы не находитесь ни в одном регионе. " +
                        "Вместо этого используется глобальный регион этого мира.");
                return global;
            }
            throw new CommandException(
                    "Вы не находитесь в регионе. " +
                            "Укажите ID, если хотите выбрать конкретный регион.");
        } else if (set.size() > 1) {
            boolean first = true;
    ...
    protected static Region checkSelection(Actor actor) throws CommandException {
        ...
        try {
            ...
            throw new CommandException("Сначала выберите область. " +
    ...
    protected static void checkRegionDoesNotExist(RegionManager manager, String id, boolean mayRedefine) throws CommandException {
        if (manager.hasRegion(id)) {
            throw new CommandException("Регион с таким именем уже существует. Пожалуйста, выберите другое имя." +
                    (mayRedefine ? " Чтобы изменить форму, используйте команду /region redefine " + id + "." : ""));
        }
    }
    ...
    protected static RegionManager checkRegionManager(World world) throws CommandException {
        if (!WorldGuard.getInstance().getPlatform().getGlobalStateManager().get(world).useRegions) {
            throw new CommandException("Поддержка регионов отключена в этом мире. " +
                    "Вы можете включить её в конфигурационных файлах WorldGuard для каждого мира отдельно. " +
                    "Однако может потребоваться перезапустить сервер.");
        }

        RegionManager manager = WorldGuard.getInstance().getPlatform().getRegionContainer().get(world);
        if (manager == null) {
            throw new CommandException("Не удалось загрузить данные регионов для этого мира. " +
                    "Попросите администратора сервера проверить логи, чтобы выяснить причину.");
        }
        return manager;
    }
    ...
    protected static ProtectedRegion checkRegionFromSelection(Actor actor, String id) throws CommandException {
        ...
        if (selection instanceof Polygonal2DRegion) {
            ...
            throw new CommandException("Извините, но для регионов WorldGuard можно использовать только кубоиды и многоугольники.");
        }
    }
    ...
    protected static void warnAboutSaveFailures(Actor sender) {
        ...
        if (!failures.isEmpty()) {
            ...
            sender.print(TextComponent.of("(Предупреждение: Фоновое сохранение данных регионов не работает для следующих миров: " + failingList + ". " +
                    "Ваши изменения теряются. Подробнее смотрите в логах сервера.)", TextColor.GOLD));
        }
    }
    ...
    protected static void warnAboutDimensions(Actor sender, ProtectedRegion region) {
        ...
        if (height <= 2) {
            sender.printDebug("(Предупреждение: Высота региона составила " + (height + 1) + " блок(ов).)");
        }
    }
    ...
    protected static void informNewUser(Actor sender, RegionManager manager, ProtectedRegion region) {
        if (manager.size() <= 2) {
            sender.print(SubtleFormat.wrap("(Этот регион теперь ЗАЩИЩЁН от изменений другими. Не хотите этого? Используйте ")
        ...
        if (spawn != null) {
            if (!spawn.getIntersectingRegions(ImmutableList.of(region)).isEmpty()) {
                sender.print(ErrorFormat.wrap("Внимание!")
                        .append(TextComponent.of(" Этот регион пересекается с защитой спавна ванильного мира. " +
                                "WorldGuard не может её переопределить, и только операторы сервера смогут взаимодействовать с этой областью.", TextColor.WHITE)));
                return true;
            }
        }
        return false;
