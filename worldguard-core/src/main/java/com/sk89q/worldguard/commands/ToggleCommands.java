...
public class ToggleCommands {
    ...
    @Command(aliases = {"stopfire"}, usage = "[<мир>]",
            desc = "Временно отключает распространение огня", max = 1)
    ...
    public void stopFire(CommandContext args, Actor sender) throws CommandException {
        ...
        if (!wcfg.fireSpreadDisableToggle) {
            worldGuard.getPlatform().broadcastNotification(
                    LabelFormat.wrap("Глобальное распространение огня в мире '" + world.getName() + "' отключено пользователем "
                    + sender.getDisplayName() + "."));
        } else {
            sender.print("Глобальное распространение огня уже было отключено");
        }
    ...
    public void allowFire(CommandContext args, Actor sender) throws CommandException {
        ...
        if (wcfg.fireSpreadDisableToggle) {
            worldGuard.getPlatform().broadcastNotification(LabelFormat.wrap("Глобальное распространение огня в мире '" + world.getName() + "' включено пользователем "
                    + sender.getDisplayName() + "."));
        } else {
            sender.print("Глобальное распространение огня уже было включено");
        }
    ...
    @Command(aliases = {"halt-activity", "stoplag", "haltactivity"}, usage = "[подтвердить]",
            desc = "Попытка остановить как можно больше активности, чтобы прекратить лаги", flags = "cis", max = 1)
        ...
        if (args.hasFlag('i')) {
            ...
            if (activityHaltToggle && (args.argsLength() == 0 || !args.getString(0).equalsIgnoreCase("confirm"))) {
                ...
                TextComponent message = TextComponent.builder("")
                        ...
                        .append(ErrorFormat.wrap(" чтобы подтвердить."))
                        .build();
            ...
            if (activityHaltToggle) {
                if (!(sender instanceof LocalPlayer)) {
                    sender.print("Вся интенсивная активность сервера приостановлена.");
                }

                if (!args.hasFlag('s')) {
                    worldGuard.getPlatform().broadcastNotification(LabelFormat.wrap("Вся интенсивная активность сервера приостановлена пользователем " + sender.getDisplayName() + "."));
                } else {
                    sender.print("(Без уведомлений) Вся интенсивная активность сервера приостановлена пользователем " + sender.getDisplayName() + ".");
                }
                ...
                if (!args.hasFlag('s')) {
                    ...
                    sender.print("(Без уведомлений) Вся интенсивная активность сервера теперь разрешена.");
                }
            }
        }
    }
