...
public abstract class RegionContainer {
    ...
    public void migrate(Migration migration) throws MigrationException {
        ...
        synchronized (lock) {
            try {
                WorldGuard.logger.info("Выгрузка и сохранение текущих данных регионов...");
                unload();
                migration.migrate();
            } finally {
                WorldGuard.logger.info("Загрузка данных регионов для загруженных миров...");
                loadWorlds();
            }
        }
    }
    ...
    protected void loadWorlds() {
        WorldGuard.logger.info("Загрузка данных регионов...");
        synchronized (lock) {
    ...
    protected void autoMigrate() {
        ...
        if (config.migrateRegionsToUuid) {
            ...
            try {
                ...
                WorldGuard.logger.info("Регионы сохранены после миграции на UUID! Это не повторится, если только  " +
                        "вы не измените соответствующую настройку в конфигурации WorldGuard.");
