        ...
        synchronized (lock) {
            ...
            if (manager != null) {
                ...
                try {
                    ...
                    log.log(Level.WARNING, "Не удалось загрузить данные региона '" + name + "' (будут периодически предприниматься попытки загрузки до успешного результата)", e);
        ...
        synchronized (lock) {
            ...
            if (manager != null) {
                try {
                    ...
                    log.log(Level.WARNING, "Не удалось сохранить данные региона '" + name + "'", e);
                }
        ...
        synchronized (lock) {
            for (Map.Entry<Normal, RegionManager> entry : mapping.entrySet()) {
                ...
                try {
                    ...
                    log.log(Level.WARNING, "Не удалось сохранить данные региона '" + name + "' при выгрузке данных для всех миров", e);
                }
            }
                ...
                for (Map.Entry<Normal, RegionManager> entry : mapping.entrySet()) {
                    ...
                    try {
                        if (manager.saveChanges()) {
                            log.info("Изменения данных региона '" + name + "' были сохранены в фоне");
                        }
                        failingSaves.remove(manager);
                    } catch (StorageException e) {
                        failingSaves.add(manager);
                        log.log(Level.WARNING, "Не удалось сохранить данные региона '" + name + "' во время периодического сохранения", e);
                    } catch (Exception e) {
                        failingSaves.add(manager);
                        log.log(Level.WARNING, "Во время периодического сохранения произошла ожидаемая ошибка", e);
                    }
                }
            }
        }
        ...
        public void run() {
            synchronized (lock) {
                if (!failingLoads.isEmpty()) {
                    log.info("Попытка загрузить данные регионов, которые ранее не удалось загрузить...");
                    ...
                    while (it.hasNext()) {
                        ...
                        try {
                            ...
                            mapping.put(normal, manager);
                            it.remove();
                            log.info("Данные региона '" + normal.toString() + " успешно загружены'");
                        } catch (StorageException e) {
                            if (e.getCause() != null && e.getCause().getMessage().equals(lastMsg)) {
                                // if it's the same error, don't print a whole stacktrace
                                log.log(Level.WARNING, "Данные региона по-прежнему не удаётся загрузить, по крайней мере для мира с именем '" + normal.toString() + "'");
                                break;
                            }
                            log.log(Level.WARNING, "Данные региона по-прежнему не удаётся загрузить, по крайней мере для мира с именем '" + normal.toString() + "'", e);
