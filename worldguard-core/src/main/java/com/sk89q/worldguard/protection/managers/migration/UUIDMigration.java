...
public class UUIDMigration extends AbstractMigration {
    ...
    protected void migrate(RegionDatabase store) throws MigrationException {
        log.log(Level.INFO, "Миграция регионов в мире '" + store.getName() + "': преобразование имён в UUID...");
    ...
    private boolean migrate(Collection<ProtectedRegion> regions) throws MigrationException {
        ...
        if (!names.isEmpty()) {
            ...
            try {
                ...
                log.log(Level.INFO, "Преобразование " + names.size() + " имени(ён) в UUID... это может занять некоторое время.");
                ...
                throw new MigrationException("Сервис преобразования имён в UUID завершился с ошибкой", e);
            ...
            log.log(Level.INFO, "UUID-адреса получены... теперь выполняется миграция всех регионов на UUID, где это возможно...");
            convert(regions);
        ...
        if (!unresolvedNames.isEmpty()) {
            if (keepUnresolvedNames) {
                log.log(Level.WARNING,
                        "Некоторые имена владельцев и участников, похоже, не существуют или не принадлежат аккаунтам Minecraft, поэтому они не были преобразованы в UUID. " +
                                "Эти имена оставлены как есть, но вы можете повторить конвертацию, установив параметр 'keep-names-that-lack-uuids' в значение false в конфигурации, " +
                                "чтобы удалить такие имена. Если оставить их, кто-то может в будущем зарегистрироваться с таким именем и получить доступ как этот игрок.");

            } else {
                log.log(Level.WARNING,
                        "Некоторые имена владельцев и участников, похоже, не существуют или не принадлежат аккаунтам Minecraft, поэтому они не были преобразованы в UUID. " +
                                "Эти имена были удалены.");
            }
        }
    }
    ...
    private class ResolvedNamesTimerTask extends TimerTask {
        @Override
        public void run() {
            log.info("UUID-адреса найдены для " + resolvedNames.size() + " имени(ён)...");
        }
    }
