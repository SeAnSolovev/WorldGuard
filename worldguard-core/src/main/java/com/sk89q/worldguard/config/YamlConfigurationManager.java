...
public abstract class YamlConfigurationManager extends ConfigurationManager {
    ...
    public void load() {
        ...
        try {
            ...
            log.severe("Ошибка чтения конфигурации глобальных настроек: ");
            e.printStackTrace();
        }
        ...
        if (!useSqlDatabase) {
            ...
            log.warning("Поддержка SQL для хранения регионов WorldGuard устаревает и будет удалена в будущей версии. Пожалуйста, перейдите на хранение в YAML.");
            log.warning("Подробнее смотрите на https://worldguard.enginehub.org/en/latest/regions/storage/");
        }
