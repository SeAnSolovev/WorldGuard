...
public class ConfigReport extends DataReport {
    ...
    public ConfigReport() {
        super("Конфигурация WorldGuard");

        List<? extends World> worlds = WorldEdit.getInstance().getPlatformManager().queryCapability(Capability.GAME_HOOKS).getWorlds();

        append("Конфигурация", new HierarchyObjectReport("Конфигурация", WorldGuard.getInstance().getPlatform().getGlobalStateManager()));
        ...
        for (World world : worlds) {
            ...
            report.append("Конфигурация", new HierarchyObjectReport("Конфигурация", config));
            ...
            if (regions != null) {
                ...
                if (global != null) {
                    ...
                    section.append("__global__", "<Не определено>");
                }

                report.append(section.getTitle(), section);
            } else {
                report.append("Регионы", "<Отключено>");
            }
