...
public class BukkitDebugHandler implements DebugHandler {
    ...
    private <T extends Event & CancelLogging> void testEvent(CommandSender receiver, Player target, T event, boolean stacktraceMode) throws
        ...
        if (!receiver.equals(target)) {
            if (!isConsole) {
                log.info(receiver.getName() + " симулирует событие для " + target.getName());
            }

            target.sendMessage(
                    ChatColor.RED + "(Пожалуйста, игнорируйте сообщения, которые могут появиться далее.)");
        }
        ...
        if (stacktraceMode) {
            ...
            log.info("Отчёт о событии для " + receiver.getName() + ":\n\n" + result);
            ...
            ActorCallbackPaste.pastebin(WorldGuard.getInstance().getSupervisor(), plugin.wrapCommandSender(receiver),
                    result, "Отчёт отладки событий: %s.txt");
        } else {
        ...
        if (fromTarget) {
            ...
            if (sender instanceof Player) {
                ...
                throw new CommandException(
                        "Если эта команда не используется в игре, используйте -t для запуска теста от имени указанного игрока, а не от себя.");
            }
        }
    }
    ...
    private Block traceBlock(CommandSender sender, Player target, boolean fromTarget) throws CommandException {
        ...
        throw new CommandException("В данный момент вы не смотрите на блок, который находится достаточно близко.");
    }
    ...
    private Entity traceEntity(CommandSender sender, Player target, boolean fromTarget) throws CommandException {
        ...
        throw new CommandException("В данный момент вы не смотрите на существо, которое находится достаточно близко.");
    }
    ...
    public void testBreak(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        sender.print(TextComponent.of("Тестирование РАЗРУШЕНИЯ БЛОКА в ", TextColor.AQUA).append(TextComponent.of(block.toString(), TextColor.DARK_AQUA)));
    ...
    public void testPlace(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        Block block = traceBlock(bukkitSender, bukkitTarget, fromTarget);
        sender.print(TextComponent.of("Тестирование УСТАНОВКИ БЛОКА в ", TextColor.AQUA).append(TextComponent.of(block.toString(), TextColor.DARK_AQUA)));
    ...
    public void testInteract(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        Block block = traceBlock(bukkitSender, bukkitTarget, fromTarget);
        sender.print(TextComponent.of("Тестирование ВЗАИМОДЕЙСТВИЯ С БЛОКОМ в ", TextColor.AQUA).append(TextComponent.of(block.toString(), TextColor.DARK_AQUA)));
    ...
    public void testDamage(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        sender.print(TextComponent.of("Тестирование УРОНА СУЩЕСТВУ в ", TextColor.AQUA).append(TextComponent.of(entity.toString(), TextColor.DARK_AQUA)));
